...
async function generateSpecSheet(heroData, outputPath) {
  ...
  // Use the canonical layout engine output so spec matches preview/exports.
  const frameBorder = layout.dimensions.moldingIn;
  const matPadding = layout.dimensions.matPaddingIn;
  const matPaddingH = layout.dimensions.matPaddingHeightIn;
  const panelGap = layout.dimensions.gapIn;

  // Mat opening = where the print sits behind the mat.
  const openingWidth = frame.width - 2 * frameBorder - 2 * matPadding;
  const openingHeight = frame.height - 2 * frameBorder - 2 * matPaddingH;

  // Print sheet size — honor explicit printW/printH (sleek/standard sizes),
  // else fall back to opening + bleed.
  ...
  const html = buildSpecHtml(heroData, frame, layoutDef, {
    frameBorder, matPadding, matPaddingH, bleed, panelGap,
...
<style>
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .diagram .opening {
    position: absolute;
    left: ${dMP}px; top: ${matPaddingH * diagramScale}px;
    width: ${dOW}px;
    height: ${dOH}px;
    background: #f4f1ea;
  ...
  <div class="grid">
    <div>
      ...
      <table class="specs-table">
        ...
        <tr><td class="label">Frame face (molding width)</td><td class="value">${frameBorder}"</td></tr>
        <tr><td class="label">Aspect ratio</td><td class="value">${frame.aspect}</td></tr>
        ${isSleek ? '<tr><td class="label">Style</td><td class="value">Sleek (no visible mat)</td></tr>' : ''}
      </table>

      <h3>Mat Dimensions</h3>
      <table class="specs-table">
        <tr><td class="label">Frame interior (inside molding)</td><td class="value">${fmt(frame.width - 2 * frameBorder)}" × ${fmt(frame.height - 2 * frameBorder)}"</td></tr>
        <tr><td class="label">Visible mat (per side)</td><td class="value">${matPadding === matPaddingH ? fmt(matPadding) + '"' : fmt(matPadding) + '" × ' + fmt(matPaddingH) + '"'}</td></tr>
        <tr><td class="label">Mat opening (where print sits)</td><td class="value">${fmt(openingWidth)}" × ${fmt(openingHeight)}"</td></tr>
      </table>
    ...
    <div>
      ...
      <div class="diagram-wrap">
        <div class="diagram">
          ...
          <div class="mat">
            <div class="dim-annotation" style="top: -12px; left: 50%; transform: translateX(-50%); font-size: 7px;">${frameBorder}" face</div>
            <div class="opening">
              ...
              ${matPadding > 0 ? `<div class="dim-annotation" style="top: -12px; left: 50%; transform: translateX(-50%); font-size: 7px;">${fmt(matPadding)}" mat</div>` : '<div class="dim-annotation" style="top: -12px; left: 50%; transform: translateX(-50%); font-size: 7px; color: #c8102e;">sleek (no mat)</div>'}
            </div>
          </div>
        </div>
      </div>
